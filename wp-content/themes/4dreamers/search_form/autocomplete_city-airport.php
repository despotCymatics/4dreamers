...
while($row = mysqli_fetch_array($rs,MYSQLI_ASSOC)) {
  $a_json_row["cssClass"] = "CityRow-disabled";
  ...
  if($airportRows){
    foreach ($airportRows as $row2) {
      if($row['CityCode'] == $row2['CityCode']) {
        ...
        $a_json_row_a["label"] = '#'.$row2['AirportName'].' | '.$row2['AirportCode'].' | '.$row['CityName'].'</span>';
...
function apply_highlight($a_json, $term) {
  ...
  for($row = 0; $row < $rows; $row++) {
    ...
    if(count($a_label_match) > 0) {
      ...
      }elseif (strpos($label, ", Airport") !== false) {
        $a_json[$row]["label"] = '<i class="fa fa-plane"></i> '.$label_highlight;
      }else {
