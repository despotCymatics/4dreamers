...
//SQL za City
$sql = 'SELECT CityName, CityCode, CountryCode, Rate, CityNameSRB FROM city WHERE Valid = 1';
$sql .= ' AND CityName LIKE "' . $con->real_escape_string($term) . '%" OR CityNameSRB LIKE "' . $con->real_escape_string($term) . '%" OR CityCode LIKE "' . $con->real_escape_string($term) . '%"';

$sqlComb = 'SELECT a.CityName, a.CityNameSRB, a.CityCode, a.CountryCode, a.Rate, b.CountryName FROM ('.$sql.') as a,';
...
$sqlComb2 = 'SELECT a.CityName, a.CityNameSRB, a.CityCode, a.CountryCode, a.Rate, b.CountryName FROM (SELECT * FROM city) as a,';
...
while($row = mysqli_fetch_array($rs,MYSQLI_ASSOC)) {
  ...
  $a_json_row["value"] = $row['CityName'].' | '.$row['CityCode']. ' | '.$row['CityNameSRB'].' | '.$row['CountryName'];
  $a_json_row["label"] = $row['CityName'].' | '.$row['CityCode']. ' | '.$row['CityNameSRB'].' | '.$row['CountryName'];
